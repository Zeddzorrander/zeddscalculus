...
\[
\frac{d}{dx} x^n = \lim_{ h\to0} \frac{(\answer[given]{x+ h})^n-x^n}{h}.
\]
...
\begin{example}
  ...
  \begin{explanation}
  	 The slope of the tangent line to the graph of the function $f(x) + g(x)$ at $x = 2$ is the derivative $ \eval{\frac{d}{dx} \left(f(x)+g(x)\right)}_{x=2}$. By the Sum Rule, this derivative is given by $f'(2) + g'(2)$.   In 
